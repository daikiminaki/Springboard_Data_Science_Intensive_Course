...
SELECT firstname, surname, MIN(joindate) AS joindate 
FROM Members 
WHERE memid > 0
...
the member name. */
SELECT CONCAT(members.firstname, ': ', facilities.name) AS tennis_bookings
...
SELECT 	
	CONCAT(facilities.name, ' used by: ',
    members.firstname),
...
SELECT * 
FROM(SELECT starttime, name, 
		CASE WHEN memid = 0 THEN slots * guestcost 
		ELSE slots * membercost END AS total_cost
	FROM (SELECT * FROM `Bookings`) AS bookings
	JOIN (SELECT * FROM `Facilities`) AS facilities
	ON bookings.facid = facilities.facid
	WHERE starttime > '2012-09-14' AND name LIKE 'tennis%') sub
WHERE total_cost > 30
